...
### 🤖 Friday – Interactive AI Voice Desktop Agent
*Designed and built a Stark-inspired custom AI desktop voice agent to automate local operating system actions.*
* **Automation & Speech**: Configured real-time, threaded speech recognition (STT) and voice feedback (TTS) scripts to process continuous voice prompts.
* **Integrations**: Integrated custom automation triggers to launch web browsers, search dynamic topics, retrieve real-time dates/times, and control local files.
* **State Management**: Built persistent JSON-based memory models allowing the assistant to store session records and custom keyword macros.
